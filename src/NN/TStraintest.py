...
class TSTrainTestSplit:
    ...
    def split(self, X):
        ...
        dat = X[self.date_idx]
        #print(dat)
        dat = dat.drop_duplicates()#.astype('datetime64[ns]')
        #print(dat)
        ...
        if self.fixed == None:
            ...
            #train_idx = dates[(dates[self.date_idx] > dat[min(train_start,len(dat)-1)])&(dates[self.date_idx] <= dat[train_end])].index
            #test_idx = dates[(dates[self.date_idx] > dat[test_start])&(dates[self.date_idx] <= dat[test_end])].index
            
                if self.shuffle:
                    train_dates = train_dates.sample(frac=1).reset_index(drop=True)
                    #np.random.shuffle(list(train_dates))
                
                yield train_dates, test_dates
                    
        else:

                #test_end_idx, test_start_idx, train_end_idx, train_start_idx = 0, int(fixed*T), int(fixed*T), T
